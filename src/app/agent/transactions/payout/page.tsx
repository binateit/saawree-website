...
import { AgentPayoutRecord } from "@/core/models/agentModel";
import { Filter, PaginationFilter } from "@/core/models/model";
import { FilterOption } from "@/core/models/saleOrderModel";
import { getPayoutListOfAgent } from "@/core/requests/agentRequests";
...
const Page = () => {
  ...
  const {
    ...
    queryKey: ["agent-payout-list", paginationModel, filterQuery],
    queryFn: () => {
      return getPayoutListOfAgent({ ...filterQuery, ...paginationModel });
    },
  });
  ...
  const filteredData = payoutsListResponse?.data?.filter(
    (item: AgentPayoutRecord) =>
  ...
  const leftContent = (
    <div className='paginaton-showing'>
      {`Showing ${
        (payoutsListResponse?.pagination?.totalCount as number) > 0
          ? (paginationModel.first as number) + 1
          : 0
      } to 
                ${Math.min(
                  (payoutsListResponse?.pagination?.currentPage as number) *
                    (payoutsListResponse?.pagination?.pageSize as number),
                  payoutsListResponse?.pagination?.totalCount as number
                )} 
                out of ${payoutsListResponse?.pagination?.totalCount} Records`}
    </div>
  );
  return (
    <>
      ...
      <div className='card shadow'>
        <div className='card-header bg-white justify-content-between'>
          <h5>Payout</h5>
          <a href='#' className='btn btn-saawree'>
            Request Payout
          </a>
        </div>
        <div className='card-body'>
          ...
          >
            ...
            <Column
              ...
              body={(rowData) => formatCurrency(rowData?.totalAmountPaid)}
            />
            <Column
            ...
            <Column
              ...
              body={(rowData) => (
                <Link
                  href={`/agent/transactions/payout/details?payoutId=${rowData?.id}`}
                  className='btn btn-saawree'
                >
                  View
                </Link>
